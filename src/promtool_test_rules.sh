...
function promtoolCheckRules {
  echo "rules: info: testing Prometheus alert rules"
  testRulesOut=$(promtool test rules ${promFiles} ${*} 2>&1)
  testRulesExitCode=${?}

  # Exit code of 0 indicates success. Print the output and exit.
  if [ ${testRulesExitCode} -eq 0 ]; then
    echo "testRules: info: Prometheus alert rule files ${promFiles} are valid."
    echo "${testRulesOut}"
    echo
  ...
  if [ ${testRulesExitCode} -ne 0 ]; then
    echo "testRules: error: Prometheus alert rule files ${promFiles} are invalid."
    echo "${testRulesOut}"
    echo
...
*Workflow: \`${GITHUB_WORKFLOW}\`, Action: \`${GITHUB_ACTION}\`, Files: \`${promFiles}\`*"
    ...
    echo "${testRulesPayload}" | curl -s -S -H "Authorization: token ${GITHUB_TOKEN}" --header "Content-Type: application/json" --data @- "${checkRulesCommentsURL}" > /dev/null
  fi
